...
public class SecurityConfig {
  ...
  public CorsConfigurationSource corsConfigurationSource(CorsProps props) {
    ...
    // Origens exatas (ex.: http://localhost:19006)
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity http, CorsConfigurationSource cors) throws Exception {
    return http
        ...
        .cors(c -> c.configurationSource(cors)) // habilita CORS no filtro de segurança
        .authorizeHttpRequests(auth -> auth
            .requestMatchers(HttpMethod.OPTIONS, "/**").permitAll() // preflight
            // libere seus endpoints públicos conforme necessário:
